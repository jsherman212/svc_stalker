...
#define WRITE_INSTR(opcode) \
    ...
    *(uint64_t *)(stalker_table + 0x8) = 0;
    ...
    puts(string_space);
    puts(sysctl_descr_loc);
    puts(sysctl_fmt_loc);
    /* for(;;); */



    /* iphone 8 13.6 for testing */
    /* WRITE_QWORD_TO_HANDLE_SVC_HOOK_CACHE(0xFFFFFFF008134654 + kernel_slide); */

    /* strcpy(string_space + sysctl_name_len + 2, "'%s' '%s'\n"); */
    /* WRITE_QWORD_TO_HANDLE_SVC_HOOK_CACHE(xnu_ptr_to_va(string_space + sysctl_name_len + 2)); */
