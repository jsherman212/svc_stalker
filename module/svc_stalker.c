...
#define WRITE_INSTR(opcode) \
    ...
    void *filter_mem = alloc_static(0x4000);

    /* stash these pointers so we have them after xnu boot */
    // XXX XXX XXX add kernel_slide?
    WRITE_QWORD(exception_triage_addr);
    WRITE_QWORD(current_proc_addr);
    WRITE_QWORD(g_proc_pid_addr);
    WRITE_QWORD(xnu_ptr_to_va(filter_mem));
    ...
    uint32_t syscall_num_to_patch = 0;
    uint8_t *sysent_struct_to_patch = NULL;
    ...
    for(uint32_t i=0; ; i++){
        ...
        if(opcodes == 0xd65f03c0528009c0){
            sysent_struct_to_patch = sysent_stream;
            syscall_num_to_patch = i;
    ...
    // and also WRITE_SVC_STALKER_SYSCALL
